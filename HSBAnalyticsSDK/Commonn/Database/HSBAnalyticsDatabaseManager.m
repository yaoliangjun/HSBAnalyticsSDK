...
static NSString * const HSBAnalyticsDefaultDatabaseName = @"HSBAnalyticsDatabase.sqlite";

@interface HSBAnalyticsDatabaseManager ()
...
@implementation HSBAnalyticsDatabaseManager {
    sqlite3 *_database;
}
...
- (instancetype)initWithFilePath:(NSString *)filePath {
    self = [super init];
    if (self) {
        _filePath = filePath ?: [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES).lastObject stringByAppendingPathComponent:HSBAnalyticsDefaultDatabaseName];

        // 初始化队列的唯一标识
        NSString *label = [NSString stringWithFormat:@"com.huishoubao.analytics.data.serial.queue.%p", self];
